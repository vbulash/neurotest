...
@push('title')
	- Редактирование теста &laquo;{{ $test->name }}&raquo;
@endpush
			...
			<div class="row mb-2">
				<div class="col-sm-6">
					<h1>@if($show)
						...
						@else
							Редактирование
						@endif теста &laquo;{{ $test->name }}&raquo;</h1>
				</div>
				<div class="col-sm-6">
	...
	<script>
		...
		@if(isset($content['branding']))
			@if(isset($content['branding']['background']))
				backgroundColor = '{{ $content['branding']['background'] }}';
			@endif
			@if(isset($content['branding']['fontcolor']))
				backgroundColor = '{{ $content['branding']['fontcolor'] }}';
			@endif
		@endif
